= rcap-rails - Common Alerting Protocol for Ruby on Rails
...
RCAP currently supports only CAP Version 1.1 and rcap-rails only supports Rails 3.X.
...
rcap-rails depends on the following gems

* {Assistance}[http://assistance.rubyforge.org]
* {UUIDTools}[http://uuidtools.rubyforge.org]
* {JSON}[http://json.rubyforge.org]

rcap-rails uses the REXML API, included in Ruby, to parse and generate XML.

== Installation

rcap-rails is distributed as a Ruby gem and is available from {Gemcutter}[http://gemcutter.org]. If you have Gemcutter set as a source of your gems then rcap-rails can be installed from the command line
